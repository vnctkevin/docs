...
# Disaster Recovery[#](#disaster-recovery "Permanent link")
...
### Configuration Backup[#](#configuration-backup "Permanent link")
...
### RonDB Backup[#](#rondb-backup "Permanent link")
...
<pre><span></span><code>/srv/hops/mysql/bin/mysqlpump<span class="w"> </span>-S<span class="w"> </span>/srv/hops/mysql-cluster/mysql.sock<span class="w"> </span>--exclude-databases<span class="o">=</span>%<span class="w"> </span>--exclude-users<span class="o">=</span>root,mysql.sys,mysql.session,mysql.infoschema<span class="w"> </span>--users<span class="w"> </span>&gt;<span class="w"> </span>users.sql
</code></pre>
...
<pre><span></span><code>/srv/hops/mysql-cluster/ndb/scripts/mgm-client.sh<span class="w"> </span>-e<span class="w"> </span><span class="s2">"START BACKUP [replace_backup_id] SNAPSHOTEND WAIT COMPLETED"</span>
</code></pre>
...
<pre><span></span><code>/srv/hops/mysql-cluster/ndb/backups/BACKUP<span class="w"> </span>-<span class="w"> </span>the<span class="w"> </span>directory<span class="w"> </span>name<span class="w"> </span>will<span class="w"> </span>be<span class="w"> </span>BACKUP-<span class="o">[</span>backup_id<span class="o">]</span><span class="w"> </span>
</code></pre>
...
### HopsFS Backup[#](#hopsfs-backup "Permanent link")
...
<pre><span></span><code>/srv/hopsworks-data/hops/hopsdata/hdfs/dn/
</code></pre>
...
## Restore[#](#restore "Permanent link")

As with the backup phase, the restore operation is broken down in different steps.

### Cluster deployment[#](#cluster-deployment "Permanent link")
...
### RonDB restore[#](#rondb-restore "Permanent link")
...
#### Restore RonDB schemas and tables[#](#restore-rondb-schemas-and-tables "Permanent link")

This command should be executed on one of the nodes in the head node group and is going to recreate the schemas, tables, and internal RonDB metadata. In the command below, you should replace the node\_id with the id of the node you are running the command on, backup\_id with the id of the backup you want to restore. Finally, you should replace the mgm\_node\_ip with the address of the node where the RonDB management service is running.

<pre><span></span><code>/srv/hops/mysql/bin/ndb_restore<span class="w"> </span>-n<span class="w"> </span><span class="o">[</span>node_id<span class="o">]</span><span class="w"> </span>-b<span class="w"> </span><span class="o">[</span>backup_id<span class="o">]</span><span class="w"> </span>-m<span class="w"> </span>--disable-indexes<span class="w"> </span>--ndb-connectstring<span class="o">=[</span>mgm_node_ip<span class="o">]</span>:1186<span class="w"> </span>--backup_path<span class="o">=</span>/srv/hops/mysql-cluster/ndb/backups/BACKUP/BACKUP-<span class="o">[</span>backup_id<span class="o">]</span>
</code></pre>

#### Restore RonDB data[#](#restore-rondb-data "Permanent link")

This command should be executed on all the RonDB datanodes. Each command should be customized with the node id of the node you are trying to restore (i.e., replace the node\_id). As for the command above you should replace the backup\_id and mgm\_node\_ip.

<pre><span></span><code>/srv/hops/mysql/bin/ndb_restore<span class="w"> </span>-n<span class="w"> </span><span class="o">[</span>node_id<span class="o">]</span><span class="w"> </span>-b<span class="w"> </span><span class="o">[</span>backup_id<span class="o">]</span><span class="w"> </span>-r<span class="w"> </span>--ndb-connectstring<span class="o">=[</span>mgm_node_ip<span class="o">]</span>:1186<span class="w"> </span>--backup_path<span class="o">=</span>/srv/hops/mysql-cluster/ndb/backups/BACKUP/BACKUP-<span class="o">[</span>backup_id<span class="o">]</span>
</code></pre>

#### Rebuild the indices[#](#rebuild-the-indices "Permanent link")

In the first command we disable the indices for recovery. This last command will take care of enabling them again. This command needs to run only once on one of the nodes of the head node group. As for the commands above, you should replace node\_id, backup\_id and mgm\_node\_id.

<pre><span></span><code>/srv/hops/mysql/bin/ndb_restore<span class="w"> </span>-n<span class="w"> </span><span class="o">[</span>node_id<span class="o">]</span><span class="w"> </span>-b<span class="w"> </span><span class="o">[</span>backup_id<span class="o">]</span><span class="w"> </span>--rebuild-indexes<span class="w"> </span>--ndb-connectstring<span class="o">=[</span>mgm_node_ip<span class="o">]</span>:1186<span class="w"> </span>--backup_path<span class="o">=</span>/srv/hops/mysql-cluster/ndb/backups/BACKUP/BACKUP-<span class="o">[</span>backup_ip<span class="o">]</span>
</code></pre>

#### Restore Users and Grants[#](#restore-users-and-grants "Permanent link")
...
<pre><span></span><code>/srv/hops/mysql-cluster/ndb/scripts/mysql-client.sh<span class="w"> </span><span class="nb">source</span><span class="w"> </span>users.sql
</code></pre>

### HopsFS restore[#](#hopsfs-restore "Permanent link")
...
<pre><span></span><code>/srv/hopsworks-data/hops/hopsdata/hdfs/dn/current
</code></pre>
...
### OpenSearch state rebuild[#](#opensearch-state-rebuild "Permanent link")
...
<pre><span></span><code>/srv/hops/epipe/bin/reindex-epipe.sh
</code></pre>

The script is deployed and configured during the platform deployment.

### Kafka topics rebuild[#](#kafka-topics-rebuild "Permanent link")
...
<pre><span></span><code>/srv/hops/kafka/bin/kafka-restore.sh
</code></pre>
